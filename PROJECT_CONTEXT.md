...
| `src/components/VoiceChat.tsx` | Interactive full-screen Voice Mode modal for **Kitty**. Plays avatar robot video (`Robot_waving_and_greeting_camera_compressed.mp4`) synchronized with browser TTS speech at exact timestamps (`2.03s`, `5.18s`, `7.18s`), captures user voice through Web Speech Recognition, and streams responses with animated glowing audio rings. |
